...
class Crud extends CI_Controller{
    ...
    public function index(){
        if ($this->session->userdata('user_login_access') != 1)
            redirect(base_url('crud/index') . 'login', 'refresh');
        if ($this->session->userdata('user_login_access') == 1)
            $data = array();
	...
	public function studlIst555()
	{
		...
		foreach ($studelist as $res) {
			...
			//$row[] = $res->id;
			$row[] = '<a href="'.$base.'/'.$res->id.'">'.$res->username.'</a>';
			$row[] = $res->email;
			$row[] = $res->designation;
			$row[] = $res->exp;
			$row[] = $res->mobileNo;
			$row[] = $res->city;
			$row[] ='<a href="'.$base.'/'.$res->id.'">View</a>';
			$row[] ='<a href="'.$resm.'/'.$res->final_file.'" style="color:blue">resume</a>';
			$row[] = $res->created_at;
			if($res->status=="active")
			{
				 $row[] ='<a class="action-buttons" id="getId" href="javascript:void(0)" onclick="return confirm("Are you sure to Deactivate this?")" data-status="inactive" data-id="'.$res->id.'" >
    ...
    public function List_employee(){
        if ($this->session->userdata('user_login_access') != False) {
            $data                  = array();
            $data['userlist']      = $this->crud_model->getAllUsersEmp();
            // echo "<pre>";
            // print_r($data);
            $data['settingsvalue'] = $this->crud_model->getSettingsValue();
            $this->load->view('backend/userlistEmp', $data);
        } else {
            redirect(base_url('crud/index'), 'refresh');
        }
    }
...
//    /*Select user information By user ID*/
        ...
        if ($this->session->userdata('user_login_access') != False) {
            ...
            $status  = $this->input->post('status');
             $data  = array(
                ...
                $res  = $this->crud_model->UserUpdate($id, $data);
               
			   if($status=='active') {
			   $output =array(
					'success'=>'Candidate Account Activate Successfully'
			   );
			   }else{
				     $output =array(
					'error'=>'Candidate Account Deactivated Successfully'
			   );
			   }
			   echo json_encode($output);
    ...
    /*user information update validation*/
    public function updateValueEmp(){
        if ($this->session->userdata('user_login_access') != False) {
            $id   = $this->input->get('id');
            $status   = $this->input->get('status');
             $data                = array(
                ...
                $success = $this->crud_model->UserUpdateEmp($id, $data);
                if($success){
                        echo "<script>alert('Employee Account Deactivated Successfully');window.location.href='List_employee';</script>";
                }else{
                    echo "<script>alert('something Went Wrong');window.location.href='List_employee';</script>";
                }
...
//    }
    ...
    public function view_brand(){
        if ($this->session->userdata('user_login_access') != False) {
            $data          = array();
            $data['brand'] = $this->crud_model->getBrand();
            $data['citys'] = $this->crud_model->getCity();
    ...
    public function Backup_Database(){
        ...
        if ($this->Backup_sql($database, $username, $password, $hostname, $path)) {
            ...
            redirect(base_url('crud/index') . 'crud/Backup_page');
        } else {
            $this->session->set_flashdata('feedback', 'Successfully Downloaded');
            redirect(base_url('crud/index') . 'crud/Backup_page');
        }
    ...
    public function salesreport(){
        //$data = '';
        $this->load->view('backend/salesreport');
    }
}
/*End crud controller*/