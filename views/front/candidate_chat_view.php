				...
				<div class="col-md-8 col-xl-6 chat" style="display:none;">
					<div class="card">
						...
						<div class="card-body msg_card_body">
							<div id="showchat" class="chat_screen"></div>
						</div>
...
$(document).ready(function(){
		...
		{
			$.ajax({  
				url:"<?php echo base_url(); ?>Home/employeechat",   
				method:"POST",  
				...
				{	
					...
					for(i=0;i<res.length;i++)
					{
						...
						{
							html +='<li><div class="d-flex bd-highlight"><div class="img_cont"><a class="gtchat" style=" text-decoration: none;" data-id="'+res[i].empid+'" data-candid="'+res[i].candid+'" href="javascript:void(0)">'+
						    	res[i].image+'<span class="online_icon"></span><span class="online_icon"></span></div>'+	
							'<div class="user_info"><span>'+res[i].employee + ' <sup style="color:#fff;font-size:bold;background-color:#4cd137; border-radius: 100px;" id="chtcount">'+res[i].chtcount+' </sup></span><p>'+res[i].employee+' is online</p></a></div></div></li>'; 
						}else{
							...
							'<div class="user_info"><span>'+res[i].employee + ' <sup style="color:#fff;font-size:bold;background-color:#4cd137;border-radius: 100px;" id="chtcount">'+res[i].chtcount+' </sup></span><p>'+res[i].employee+' is offline</p></a></div></div></li>'; 
		...
		$(document).on('click','.gtchat',function(){
			...
			var candid = $(this).data('candid');
				...
				$('.chat').show();
			$.ajax({  
					url:"<?php echo base_url(); ?>Home/chtrecordsingemp",   
					method:"POST",  
					...
					success:function(res)  
					{
						...
						if(res[i].login_status=='1')
						{
							...
							'<div class="img_cont">'+res[i].eimage+
							'<span class="online_icon"></span></div><div class="user_info">	<span>'+res[i].employee+'</span><p>1767 Messages</p></div></div>'+
							...
							'<div class="d-flex bd-highlight">'+
							'<div class="img_cont">'+res[i].eimage+
							'<span class="online_icon offline"></span></div><div class="user_info">	<span>'+res[i].employee+'</span><p>1767 Messages</p></div>	</div>'+
							'<span id="action_menu_btn"><i class="fas fa-ellipsis-v"></i></span><div class="action_menu"><ul><li><i class="fas fa-user-circle"></i> View profile</li>'+
							'<li><i class="fas fa-users"></i> Add to close friends</li>	<li><i class="fas fa-plus"></i> Add to group</li><li><i class="fas fa-ban"></i> Block</li></ul></div></div>';
						}	
						for(i=0;i<res.length;i++)
						{
							if(res[i].employee_response_message=='')
							{
									...
									'<div class="msg_cotainer_send">'+res[i].candidate_send_message+'<span class="msg_time_send">'+res[i].chat_time+'</span>'+
									...
									'<div class="msg_cotainer">'+res[i].employee_response_message+' <br /><span class="msg_time">'+res[i].chat_time+'</span></div></div>'	;
		...
		if($('#message').val()!='')
		{
			...
			setInterval(function(){ 
				...
				$.ajax({  
					url:"<?php echo base_url(); ?>Home/livechat",   
					method:"POST",  
					data:{empid:empid,candid:candid},  
					//contentType: false,  
					//cache: false,  
		...
		$(document).on('click','.gtchat',function(){
			var empid = $(this).data('id');
			$.ajax({  
					url:"<?php echo base_url(); ?>Home/chatupdatestatus",   
					method:"POST",  
					data:{empid:empid},  
					//contentType: false,  
					//cache: false,  
